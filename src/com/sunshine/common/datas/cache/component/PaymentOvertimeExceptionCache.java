 ...
 * @Version: 1.0
 */
public class PaymentOvertimeExceptionCache {
	private static Logger logger = LoggerFactory.getLogger(PaymentOvertimeExceptionCache.class);
	private RedisService redisSvc = SpringContextHolder.getBean(RedisService.class);

	public void initCache() {
		logger.info("加载支付超时异常订单缓存start.................");
		TradeOrderService orderService = SpringContextHolder.getBean(TradeOrderService.class);
		List<TradeOrder> orders = orderService.getAllPaymentOvertimeException();
	...
	public ExceptionTradeOrder getExceptionPayTradeOrderFromCache(String appId) {
		...
		//判断是否未空 或者是否存在
		if (StringUtils.isNotEmpty(val) && !"null".equalsIgnoreCase(val)) {
	...
	public ExceptionTradeOrder removeExceptionPayTradeOrderFromCache(String appId) {
		...
		//判断是否未空 或者是否存在
		if (StringUtils.isNotEmpty(jsonVal) && !"null".equalsIgnoreCase(jsonVal)) {
	...
	public String getCacheKey(String appId) {
		return CacheConstants.CACHE_PAYMENT_OVERTIME_EXCEPTION_REFIX.concat(appId);
	}
}
