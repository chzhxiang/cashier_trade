...
import com.sunshine.task.biz.callable.HandleOrderOvertimeExceptionCollectorCall;
import com.sunshine.task.biz.vo.TaskResultInfo;

/**
 * @Project: cashier_trade 
 * @Package: com.sunshine.task.biz.collector
 * @ClassName: HandleOrderOvertimeExceptionCollector
 ...
 * @Version: 1.0
 */
public class HandleOrderOvertimeExceptionCollector {
	public static Logger logger = LoggerFactory.getLogger(HandleOrderOvertimeExceptionCollector.class);
	...
	public void start() {
		...
		if (!CollectionUtils.isEmpty(merchants)) {
			...
			logger.info("需处理超时订单应用:{}", JSON.toJSONString(applications));
		} else {
			logger.info("需处理超时订单商户为空");
		}
		...
		if (threadNum > 0) {
			// 设置线程池的数量
			ExecutorService collectExec = Executors.newFixedThreadPool(threadNum, new SimpleThreadFactory("payOvertimeOrderExce"));
			// map中的key为2种
			...
			for (MerchantApplication application : applications) {
				HandleOrderOvertimeExceptionCollectorCall collectCall = new HandleOrderOvertimeExceptionCollectorCall(application);
				// 创建每条指令的采集任务对象
